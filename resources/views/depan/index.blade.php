        ...
        <div class="container-fluid p-0">
                    ...
                    @foreach($experience as $ex)
                        <div class="d-flex flex-column flex-md-row justify-content-between mb-5">
                            <div class="flex-grow-1">
                                <h3 class="mb-0">{{$ex->judul}}</h3>
                                <div class="subheading mb-3">{{$ex->info1}}</div>
                                {!! $ex->isi !!}
                            </div>
                            <div class="flex-shrink-0"><span class="text-primary">{{$ex->tgl_mulai_indo}} - {{$ex->tgl_akhir_indo}}</span></div>
                        </div>
                    @endforeach
                </div>
            </section>
            ...
            <section class="resume-section" id="education">
                <div class="resume-section-content">
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between mb-5">
                        <div class="flex-grow-1">
                            <h3 class="mb-0">{{$ed->judul}}</h3>
                            <div class="subheading mb-3">{{$ed->info1}}</div>
                            <div>{{$ed->info2}}</div>
                            <p>GPA: {{$ed->info3}}</p>
                        </div>
                        <div class="flex-shrink-0"><span class="text-primary">{{$ed->tgl_mulai_indo}} - {{$ed->tgl_akhir_indo}}</span></div>
                    </div>
                    @endforeach()
                </div>
            </section>
            ...
            <section class="resume-section" id="skills">
                <div class="resume-section-content">
                    ...
                    <ul class="list-inline dev-icons">
                        @foreach(explode(', ',get_meta_value('_language')) as $sk)
                        <li class="list-inline-item"><i class="devicon-{{strtolower($sk)}}-plain"></i></li>
                        @endforeach
                    </ul>
